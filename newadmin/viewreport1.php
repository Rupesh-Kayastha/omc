...
$sql1="SELECT * FROM monitoring_list WHERE date_entry BETWEEN '".$formd."' and '".$todate."' and  lid=$location and env_id=$lid and pid=$pid and userid=$user";
$query1=$cudb->query($sql1) or die(mysqli_error()); 
}
}
...
<td><?php echo $result1->remark; ?></td>
 ...
 <tr>
  ...
  <td><?=$a=max($resp['pm10']);?></td>
  <td><?=$a=max($resp['pm25']);?></td>
  <td><?=$a=max($resp['so2']);?></td>
  <td><?=$a=max($resp['nox']);?></td>
  <td><?=$a=max($resp['co']);?></td>
  <td>&nbsp;</td>
 </tr>
 <tr>
  <td colspan="2">Minimum</td>
  <td><?=min(array_map('floatval',array_filter($resp['pm10'])));?></td>
  <td><?=min(array_filter($resp['pm25']));?></td>
  <td><?=min(array_filter($resp['so2']));?></td>
  <td><?=min(array_filter($resp['nox']));?></td>
  <td><?=min(array_filter($resp['co']));?></td>
  <td>&nbsp;</td>
 </tr>
 <tr>
  <td colspan="2">Average</td>
  <td><?=round((array_sum($resp['pm10'])/$i),2);?></td>
  <td><?=round((array_sum($resp['pm25'])/$i),2);?></td>
  <td><?=round((array_sum($resp['so2'])/$i),2);?></td>
  <td><?=round((array_sum($resp['nox'])/$i),2);?></td>
  <td><?=round((array_sum($resp['co'])/$i),2);?></td>
  <td>&nbsp;</td>
 </tr>
 <tr>
...
$sql1="SELECT * FROM fugitive where env_id=$lid and pid=$pid and userid=$user $quy_con";
$query1=$cudb->query($sql1) or die(mysqli_error());
?>
...
if(isset($_POST['location']) && ! ($_POST['location']=='all'|| $_POST['location']==''))
{
   $quy_con.= " and LocationId= $_POST[location]";
    ...
    $sql1="SELECT * FROM ground_water_level where lid=$lid and pid=$pid and UserId=$user $quy_con";
$query1=$cudb->query($sql1) or die(mysqli_error());
?>
...
$quy_con='';
if((isset($_POST['formd'])&&$_POST['formd']!='')&& (isset($_POST['todate'])&&$_POST['todate']!='')){
  $quy_con.=  " and date(OnDate) >='$_POST[formd]' and date(OnDate) <='$_POST[todate]'";
...
while($result1=mysqli_fetch_object($query1))
{
$OnDate=date('Y-m-d',strtotime($result1->OnDate));
 $resp[$OnDate][$result1->LocationId][]=$result1->MonitoringValue;
 $resd[$OnDate][$result1->LocationId]=$result1->MontoringDate;
 $rest[$OnDate][$result1->MonitoringTime][$result1->LocationId]=$result1->MonitoringValue;
 $loca[$result1->LocationId]=$result1->LocationId;
 $datadate[$OnDate]=$OnDate;
 $i=$i+1;
}
...
<tr>
 ...
 <?php
 foreach($resd[$dval] as $key=>$value)
{
 ?>
...
$sql1="SELECT * FROM AmbientNoiseDay where Lid=$lid and Pid=$pid and UserId=$user $quy_con order by OnDate desc";
$query1=$cudb->query($sql1) or die(mysqli_error());
$i=0;
$rest=array();$resp==$loca=$datadate=array();
...
<table class="table table-striped table-bordered" id="fugitive">
 <thead>
  <tr>
   <th colspan="<?=count($loca)+4;?>"><h3>Ambient Noise Level Monitoring (Day Time) <!-- : <?php echo $dval ?> --> </h3></th>
  </tr>
 </thead>
<thead>
  <tr>   
...
<tr>
 ...
 {
 ?>
 <td><?php echo $value ?></td><!-- <?=round((array_sum($resp[$dval][$value])/count($resp[$dval][$value])),2)?> -->
 <?php
 }
 ?>
</tr>
...
$sql1="SELECT * FROM AmbientNoiseNight where Lid=$lid and Pid=$pid and UserId=$user $quy_con order by OnDate desc";
$query1=$cudb->query($sql1) or die(mysqli_error());
$i=0;
$rest=array();$resp==$loca=$datadate=array();
...
{
 $OnDate=date('Y-m-d',strtotime($result1->OnDate));
 $resp[$OnDate][$result1->LocationId][]=($result1->MonitoringValue=='BDL')?30:$result1->MonitoringValue;
 $resd[$OnDate][$result1->LocationId]=$result1->MontoringDate;
 $rest[$OnDate][$result1->MonitoringTime][$result1->LocationId]=($result1->MonitoringValue=='BDL')?30:$result1->MonitoringValue;
 $loca[$result1->LocationId]=$result1->LocationId;
 $datadate[$OnDate]=$OnDate;
 $i=$i+1;
}
...
<tr>
 ...
 <td><?=$tval;?></td>
  ...
  {
   ?>
  <td><?=($rest[$dval][$tval][$value]=='BDL')?30:$rest[$dval][$tval][$value] ?></td>
  <?php
  }
 ?>
</tr>
...
<tr>
 ...
 {
 ?>
 <td><?=round((array_sum($resp[$dval][$value])/count($resp[$dval][$value])),2)?></td>
 <?php
 }
 ?>
</tr>
...
$sql1="SELECT * FROM SurfaceWaterAnalysis where Lid=$lid and Pid=$pid and UserId=$user $quy_con";
$query1=$cudb->query($sql1) or die(mysqli_error());
$i=0;
$rest=array();$resp==$loca=$datadate=array();
...
{
  $OnDate=date('Y-m-d',strtotime($result1->OnDate));
 $resp[$OnDate][$result1->LocationId][]=$result1->SamplingValue;
 $resd[$OnDate][$result1->LocationId]=$result1->SamplingDate;
 $rest[$OnDate][$result1->SamplingId][$result1->LocationId]=$result1->SamplingValue;
 $loca[$result1->LocationId]=$result1->LocationId;
 $datadate[$OnDate]=$OnDate;
 $i=$i+1;
}
...
<table class="table table-striped table-bordered" id="fugitive">
 <thead>
  <tr>
   <th colspan="<?=count($loca)+2;?>"><h3>Results Of Surface Water Analysis</h3></th>
  </tr>
 </thead>
<thead>
  <tr>   
...
$sql1="SELECT * FROM FlowWaterMeasurement where Lid=$lid and Pid=$pid and UserId=$user $quy_con";
$query1=$cudb->query($sql1) or die(mysqli_error());
?>
...
$sql1="SELECT * FROM GroundWaterQuality where GroundType='A' and Lid=$lid and Pid=$pid and UserId=$user $quy_con";
$query1=$cudb->query($sql1) or die(mysqli_error());
$i=0;
$rest=array();$resp==$loca=$datadate=array();
...
$sql1="SELECT * FROM GroundWaterQuality where GroundType='B' and Lid=$lid and Pid=$pid and UserId=$user $quy_con";
$query1=$cudb->query($sql1) or die(mysqli_error());
$i=0;
$rest=array();$resp==$loca=$datadate=array();
...
if(isset($_POST['location']) && ! ($_POST['location']=='all'|| $_POST['location']==''))
{
 ...
 $sql111="SELECT * FROM GroundWaterQuality where GroundType='C' and Lid=$lid and Pid=$pid and UserId=$user $quy_con";
$query111=$cudb->query($sql111) or die(mysqli_error());
$i=0;
$rest=array();$resp==$loca=$datadate=array();

while($result111=mysqli_fetch_object($query111))
{
 $rest[$result111->SamplingDate][$result111->SamplingId][$result111->LocationId]=$result111->SamplingValue;
 $loca[$result111->LocationId]=$result111->LocationId;
 $datadate[$result111->SamplingDate]=$result111->SamplingDate;
 $i=$i+1;
}
?>  
...
$sql1="SELECT * FROM GroundWaterQuality1 where  Lid=$lid and Pid=$pid and UserId=$user $quy_con";
$query1=$cudb->query($sql1) or die(mysqli_error());
$i=0;
$rest=array();$resp==$loca=$datadate=array();
...
if(isset($_REQUEST['submit']))
{
  ...
  if($location=='all')
  {
$sql1="SELECT * FROM PersonalDust where Lid=$lid and pid=$pid and UserId=$user";
$query1=$cudb->query($sql1) or die(mysqli_error()); 
...
$sql1="SELECT * FROM PersonalDust WHERE Date BETWEEN '".$formd."' and '".$todate."' and  LocationId=$location and Lid=$lid and pid=$pid and UserId=$user";
$query1=$cudb->query($sql1) or die(mysqli_error()); 
}
}
else
...
<td><?php echo $result1->MEL; ?></td>
 ...
 <section class="content">
       ...
       <div class="row">
        ...
        <div class="col-md-12 np">
         <div class="main-cnt">
          <h3 class="box-title">Surface Runoff Water Quality</h3>
 <?php
$quy_con='';
if((isset($_POST['formd'])&&$_POST['formd']!='')&& (isset($_POST['todate'])&&$_POST['todate']!='')){
  $quy_con.=  " and SamplingDate >='$_POST[formd]' and SamplingDate <='$_POST[todate]'";
...
$sql1="SELECT * FROM SurfaceRunoff where Lid=$lid and Pid=$pid and UserId=$user $quy_con";
$query1=$cudb->query($sql1) or die(mysqli_error());
$i=0;
$rest=array();$resp==$loca=$datadate=array();
...
?>
   <table class="table table-striped table-bordered" id="fugitive">
    <thead>
  <tr>
      <th colspan="<?=count($loca)+3;?>"><h3>Surface Runoff Water Quality</h3></th>
  </tr>
  <tr>
   <th rowspan="2">Parameter</th>
   <th rowspan="2">Unit </th>
   <th rowspan="2">Prescribed Standard</th>
   <?php
      $i=0;
        foreach($loca as $key=>$value)
       {
...
$sql1="SELECT * FROM ground_water_quality where Lid=$lid and Pid=$pid and UserId=$user $quy_con";
$query1=$cudb->query($sql1) or die(mysqli_error());
$i=0;
$rest=array();$resp==$loca=$datadate=array();
...
?>
   <table class="table table-striped table-bordered" id="fugitive">
    <thead>
  <tr>
      <th colspan="<?=count($loca)+2;?>"><h3>ground water quality</h3></th>
  </tr>
  <tr>
   <th rowspan="2">location</th>
   <th rowspan="2">Cr+6 conc.(mg/l)</th>
   <?php
      $i=0;
        foreach($loca as $key=>$value)
       {
...
?>
 ...
 <section class="content">
       ...
       <div class="row">
        ...
        <div class="col-md-12 np">
         <div class="main-cnt">
          <h3 class="box-title">Soil Quality Analysis</h3>
 <?php
$quy_con='';
if((isset($_POST['formd'])&&$_POST['formd']!='')&& (isset($_POST['todate'])&&$_POST['todate']!='')){
  $quy_con.=  " and SamplingDate >='$_POST[formd]' and SamplingDate <='$_POST[todate]'";
...
$sql1="SELECT * FROM soilquality where Lid=$lid and Pid=$pid and UserId=$user $quy_con";
$query1=$cudb->query($sql1) or die(mysqli_error());
$i=0;
$rest=array();$resp==$loca=$datadate=array();
...
?>
   <table class="table table-striped table-bordered" id="fugitive">
    <thead>
  <tr>
      <th colspan="<?=count($loca)+2;?>"><h3>Soil Quality Analysis</h3></th>
  </tr>
  <tr>
   <th rowspan="2">Parameter</th>
   <th rowspan="2">Unit </th>
...
<?php
 ...
 <?php
	...
	<?php
      ...
      {
        ?>
<td rowspan="2"><?=$Exchangeable[$i];?></td>
				<?php
...
if(isset($_POST['location'])&& ! ($_POST['location']=='all'|| $_POST['location']==''))
{
   $quy_con.= " and LocationId= $_POST[location]";
    ...
    $sql1="SELECT * FROM VehicularEmission where lid=$lid and pid=$pid and UserId=$user $quy_con";
$query1=$cudb->query($sql1) or die(mysqli_error());
?>
