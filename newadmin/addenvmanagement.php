...
if($editcondid)
{
$inscllist="UPDATE EnviornmentalManagement SET SiteId='$pid', ManagementCondtnName='$managementcondtn',Frequency='$frequency', Description='$description',Responsibility='$userlist' WHERE EnvManagementId=$editcondid";
...
{
$inscllist="INSERT INTO EnviornmentalManagement SET SiteId='$pid', ManagementCondtnName='$managementcondtn',Frequency='$frequency', Description='$description',Responsibility='$userlist',`OnDate`='$date'";
  $res=$cudb->query($inscllist) or die(mysqli_error());
   echo "<div class='alert alert-success fade in alert-dismissable'>
...
if($editcondid) {
$sqlpr="SELECT * FROM EnviornmentalManagement WHERE EnvManagementId='$editcondid'";
        ...
        <div class="col-md-12">
          ...
          <div class="box main-cnt">
            <div class="box-header with-border">
              <h3 class="box-title">Add Enviornmental Management</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
          ...
          <?php
                ...
                while($result=mysqli_fetch_object($query))
                    ...
                    $i=$i+1;
            ?> 
                <option <?php if($result->id==$resultp->SiteId) {  echo "selected"?><? } ?> value="<?php echo $result->id;?>"><?php echo $result->p_name;?></option>
            <?php
            } ?>
          </select>
          
          <input type="text" name="managementcondtn" class="form-control" value="<?=$resultp->ManagementCondtnName;?>" placeholder="Management Condition Name"/>
          <div class="form-group" style="width:100%; float:left;">

                <div id="type_container">
              <div class="form-group" id="edit-0" style="width:100%; float:left;">
                  <textarea class="tinymce-enabled-message" name="description" cols="80" id="description1" placeholder="Description" required=""><?php echo $resultp->Description; ?></textarea>
                   ...
                   <select name="frequency" class="form-control" id="changecl" required>
                    <option value="">Update Frequency</option>
                    <option value="Daily" <?php if($resultp->Frequency=='Daily'){ echo "selected";} ?>> Daily</option>
                    <option value="Monthly" <?php if($resultp->Frequency=='Monthly'){ echo "selected";} ?>> Monthly</option>
				  </select>
                
                Responsibility <br/>
                <select name="user[]" class="form-control" id="changecl" multiple required>
                    <?php
                $userlist=explode('|',$resultp->Responsibility);
                ...
                while($result=mysqli_fetch_object($query))
                    {
            ?> 
                <option <?php if(in_array($result->id,$userlist)) {  echo "selected"?><? } ?> value="<?php echo $result->id;?>"><?php echo $result->username;?></option>
            <?php
            } ?>
				</select>
